...
func GetPendingRequests(db *DB, f PendingFilter) ([]PendingRequest, int, error) {
	...
	var total int
	err := db.ReadDB().QueryRow("SELECT COUNT(*) FROM pending_requests WHERE "+whereClause, args...).Scan(&total)
	if err != nil {
		return nil, 0, err
	}
	...
	rows, err := db.ReadDB().Query(
		`SELECT id, container_name, container_id, destination_host, destination_port,
		        resolved_hostname, first_seen, last_seen, attempt_count
		 FROM pending_requests WHERE `+whereClause+` ORDER BY last_seen DESC LIMIT ? OFFSET ?`,
		append(args, f.Limit, f.Offset)...,
	)
	if err != nil {
		return nil, 0, err
...
func GetPendingCount(db *DB) (int, error) {
	var count int
	err := db.ReadDB().QueryRow("SELECT COUNT(*) FROM pending_requests").Scan(&count)
	return count, err
}
...
func QueryLogs(db *DB, f LogFilter) ([]RequestLog, int, error) {
	...
	var total int
	err := db.ReadDB().QueryRow("SELECT COUNT(*) FROM request_logs WHERE "+whereClause, args...).Scan(&total)
	if err != nil {
		return nil, 0, err
	}
	...
	rows, err := db.ReadDB().Query(
		`SELECT id, timestamp, container_name, container_id, destination_host, destination_port,
		        resolved_hostname, method, result, rule_id, response_time_ms
		 FROM request_logs WHERE `+whereClause+` ORDER BY timestamp DESC LIMIT ? OFFSET ?`,
		append(args, f.Limit, f.Offset)...,
	)
	if err != nil {
		return nil, 0, err
	...
	for rows.Next() {
		...
		if err := rows.Scan(&l.ID, &l.Timestamp, &l.ContainerName, &l.ContainerID, &l.DestinationHost,
			&l.DestinationPort, &l.ResolvedHostname, &l.Method, &l.Result, &l.RuleID, &l.ResponseTimeMs); err != nil {
			return nil, 0, err
		}
		logs = append(logs, l)
	}
...
func GetDashboardStats(db *DB, fromDate, toDate time.Time, groupBy string, recentLimit int) (*DashboardStats, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&b.Host, &b.Port, &b.ResolvedHostname, &b.Count); err != nil {
			continue
		}
